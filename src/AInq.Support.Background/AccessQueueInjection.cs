...
namespace AInq.Support.Background
{
    public static class AccessQueueInjection
    {
        ...
        public static IServiceCollection AddAccessQueue<TResource>(this IServiceCollection services, Func<IServiceProvider, TResource> resourceFabric, ReuseStrategy strategy, int maxSimultaneous = 1)
        {
            ...
            return strategy switch
            {
                ReuseStrategy.Static => maxSimultaneous == 1
                    ...
                    : services.AddHostedService(provider => new TaskWorker<TResource, object>(provider, manager, new MultipleStaticProcessor<TResource, object>(Enumerable.Repeat(0, maxSimultaneous).Select(_ => resourceFabric.Invoke(provider))))),
        ...
        public static IServiceCollection AddPriorityAccessQueue<TResource>(this IServiceCollection services, int maxPriority, Func<IServiceProvider, TResource> resourceFabric, ReuseStrategy strategy, int maxSimultaneous = 1)
        {
            if (services.Any(service => service.ImplementationType == typeof(IAccessQueue<TResource>)))
                throw new InvalidOperationException("Service already exists");
            if (maxPriority < 0)
            ...
            return strategy switch
            {
                ReuseStrategy.Static => maxSimultaneous == 1
                    ...
                    : services.AddHostedService(provider => new TaskWorker<TResource, int>(provider, manager, new MultipleStaticProcessor<TResource, int>(Enumerable.Repeat(0, maxSimultaneous).Select(_ => resourceFabric.Invoke(provider)).Where(machine => machine != null)))),
                ReuseStrategy.Reuse => maxSimultaneous == 1
        ...
        public static IServiceCollection AddAccessQueue<TResource>(this IServiceCollection services, ReuseStrategy strategy, int maxSimultaneous = 1)
        {
            ...
            {
                ReuseStrategy.Static => maxSimultaneous == 1
                    ? services.AddHostedService(provider => new TaskWorker<TResource, object>(provider, manager, new SingleStaticProcessor<TResource, object>(provider.GetService<TResource>())))
                    : services.AddHostedService(provider => new TaskWorker<TResource, object>(provider, manager, new MultipleStaticProcessor<TResource, object>(Enumerable.Repeat(0, maxSimultaneous).Select(_ => provider.GetService<TResource>()).Where(machine => machine != null)))),
                ReuseStrategy.Reuse => maxSimultaneous == 1
                    ? services.AddHostedService(provider => new TaskWorker<TResource, object>(provider, manager, new SingleReusableProcessor<TResource, object>(serviceProvider => serviceProvider.GetService<TResource>())))
                    : services.AddHostedService(provider => new TaskWorker<TResource, object>(provider, manager, new MultipleReusableProcessor<TResource, object>(serviceProvider => serviceProvider.GetService<TResource>(), maxSimultaneous))),
                ReuseStrategy.OneTime => maxSimultaneous == 1
                    ? services.AddHostedService(provider => new TaskWorker<TResource, object>(provider, manager, new SingleOneTimeProcessor<TResource, object>(serviceProvider => serviceProvider.GetService<TResource>())))
                    : services.AddHostedService(provider => new TaskWorker<TResource, object>(provider, manager, new MultipleOneTimeProcessor<TResource, object>(serviceProvider => serviceProvider.GetService<TResource>(), maxSimultaneous))),
                _ => throw new ArgumentOutOfRangeException(nameof(strategy), strategy, null)
            };
        }
        ...
        public static IServiceCollection AddPriorityAccessQueue<TResource>(this IServiceCollection services, int maxPriority, ReuseStrategy strategy, int maxSimultaneous = 1)
        {
            ...
            {
                ReuseStrategy.Static => maxSimultaneous == 1
                    ? services.AddHostedService(provider => new TaskWorker<TResource, int>(provider, manager, new SingleStaticProcessor<TResource, int>(provider.GetService<TResource>())))
                    : services.AddHostedService(provider => new TaskWorker<TResource, int>(provider, manager, new MultipleStaticProcessor<TResource, int>(Enumerable.Repeat(0, maxSimultaneous).Select(_ => provider.GetService<TResource>()).Where(machine => machine != null)))),
                ReuseStrategy.Reuse => maxSimultaneous == 1
                    ? services.AddHostedService(provider => new TaskWorker<TResource, int>(provider, manager, new SingleReusableProcessor<TResource, int>(serviceProvider => serviceProvider.GetService<TResource>())))
                    : services.AddHostedService(provider => new TaskWorker<TResource, int>(provider, manager, new MultipleReusableProcessor<TResource, int>(serviceProvider => serviceProvider.GetService<TResource>(), maxSimultaneous))),
                ReuseStrategy.OneTime => maxSimultaneous == 1
                    ? services.AddHostedService(provider => new TaskWorker<TResource, int>(provider, manager, new SingleOneTimeProcessor<TResource, int>(serviceProvider => serviceProvider.GetService<TResource>())))
                    : services.AddHostedService(provider => new TaskWorker<TResource, int>(provider, manager, new MultipleOneTimeProcessor<TResource, int>(serviceProvider => serviceProvider.GetService<TResource>(), maxSimultaneous))),
                _ => throw new ArgumentOutOfRangeException(nameof(strategy), strategy, null)
            };
        }
    }
