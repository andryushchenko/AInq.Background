...
using static AInq.Support.Background.AccessFactory;
using static AInq.Support.Background.Elements.AccessWrapperFactory;
...
namespace AInq.Support.Background.Managers
{
    internal class AccessQueueManager<TResource> : IAccessQueue<TResource>, ITaskManager<TResource, object>
    {
        ...
        Task IAccessQueue<TResource>.EnqueueAccess<TAccess>(CancellationToken cancellation, int attemptsCount)
        {
            if (attemptsCount < 1)
                throw new ArgumentOutOfRangeException(nameof(attemptsCount), attemptsCount, null);
            var (accessWrapper, task) = CreateAccessWrapper(CreateAccess<TResource>((resource, provider) => provider.GetRequiredService<TAccess>().Access(resource, provider)), attemptsCount, cancellation);
            Queue.Enqueue(accessWrapper);
            NewAccessEvent.Set();
            return task;
        }
        ...
        Task<TResult> IAccessQueue<TResource>.EnqueueAccess<TAccess, TResult>(CancellationToken cancellation, int attemptsCount)
        {
            if (attemptsCount < 1)
                throw new ArgumentOutOfRangeException(nameof(attemptsCount), attemptsCount, null);
            var (accessWrapper, task) = CreateAccessWrapper(CreateAccess<TResource, TResult>((resource, provider) => provider.GetRequiredService<TAccess>().Access(resource, provider)), attemptsCount, cancellation);
            Queue.Enqueue(accessWrapper);
            NewAccessEvent.Set();
            return task;
        }
        ...
        Task IAccessQueue<TResource>.EnqueueAsyncAccess<TAsyncAccess>(CancellationToken cancellation, int attemptsCount)
        {
            if (attemptsCount < 1)
                throw new ArgumentOutOfRangeException(nameof(attemptsCount), attemptsCount, null);
            var (accessWrapper, task) = CreateAccessWrapper(CreateAccess<TResource>((resource, provider, token) => provider.GetRequiredService<TAsyncAccess>().AccessAsync(resource, provider, token)), attemptsCount, cancellation);
            Queue.Enqueue(accessWrapper);
            NewAccessEvent.Set();
            return task;
        }
        ...
        Task<TResult> IAccessQueue<TResource>.EnqueueAsyncAccess<TAsyncAccess, TResult>(CancellationToken cancellation, int attemptsCount)
        {
            if (attemptsCount < 1)
                throw new ArgumentOutOfRangeException(nameof(attemptsCount), attemptsCount, null);
            var (accessWrapper, task) = CreateAccessWrapper(CreateAccess<TResource, TResult>((resource, provider, token) => provider.GetRequiredService<TAsyncAccess>().AccessAsync(resource, provider, token)), attemptsCount, cancellation);
            Queue.Enqueue(accessWrapper);
            NewAccessEvent.Set();
            return task;
        }
